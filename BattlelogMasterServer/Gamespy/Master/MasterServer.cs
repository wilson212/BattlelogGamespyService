...
namespace BattlelogMaster
{
    ...
    public class MasterServer : GamespyUdpSocket
    {
        ...
        public MasterServer(IPEndPoint bindTo, LogWriter DebugLog) : base(bindTo, MaxConnections)
        {
            ...
            // Server life
            ServerTTL = 30;//Config.GetType<int>("Settings", "ServerListTTL");
        ...
        private bool ParseServerDetails(IPEndPoint remote, byte[] data)
        {
            ...
            Type serverType = typeof(GameServer);
            ...
            {
                // Fetch the property
                PropertyInfo property = serverType.GetProperty(serverVarsSplit[i]);
                if (property == null)
                {
                    continue;
                }
                ...
                else if (property.Name == "bf2_plasma")
                {
                    try
                    {
                        ...
                        using (MasterDatabase Driver = new MasterDatabase())
                        {
                            var Rows = Driver.Query("SELECT plasma FROM web_servers WHERE ip=@P0 AND queryport=@P1", remote.Address, remote.Port);
                            if (Rows.Count > 0)
